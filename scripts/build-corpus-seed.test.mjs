...
import { calculateWithCiphers, CORE_CIPHERS } from '../lib/gematria/index';
import { buildSeedSql, calculateCoreValues } from './build-corpus-seed.mjs';

describe('corpus seed builder', () => {
  it('matches the production core values for Gematria', () => {
    expect(calculateCoreValues('Gematria')).toEqual({
      'english-ordinal': 74,
      'full-reduction': 38,
      'reverse-ordinal': 142,
  ...
  it('escapes phrases and emits all core cipher rows', () => {
    ...
    expect(sql.match(/phrase_cipher_values/g)).toHaveLength(4);
    expect(sql).toContain('begin;');
    expect(sql).toContain('commit;');
  });

  it('uses the production full-number behavior', () => {
    expect(calculateCoreValues('A12')).toEqual({
      'english-ordinal': 13,
      'full-reduction': 13,
      'reverse-ordinal': 38,
  ...
  it('stays in sync with the production calculator', () => {
    for (const phrase of ['Gematria', 'A12', 'Café', 'Mars 2026']) {
      const production = Object.fromEntries(
        calculateWithCiphers(phrase, CORE_CIPHERS).map((result) => [
          result.cipherId,
          result.total
        ])
      );
      expect(calculateCoreValues(phrase)).toEqual(production);
    }
  });
